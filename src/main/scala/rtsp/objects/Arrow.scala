...
class Arrow(engine: GameEngine, val archer: RTSPWarrior, val target: RTSPWarrior) extends GameObject(engine) {
    ...
    override def onUpdate() = {
        ...
        val angle = math.atan2(target.position.y - position.y, target.position.x - position.x).toFloat
